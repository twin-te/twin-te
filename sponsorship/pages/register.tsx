		...
		Promise.all([
			...
			useCase.getSubscriptionPlans().then((subscriptionPlans) => {
				if (subscriptionPlans.length === 0)
					throw new Error("not found subscription plans");
	...
	return (
		<>
			...
			<div className="card">
				...
				<p className={`has-text-centered has-text-primary ${styles.price}`}>
					¥{donationPrices[donationPriceIndex]}
				</p>
				...
				<p className="has-text-primary">
					...
					<span style={{ fontWeight: "bold" }}>
						{Math.round(
							(Math.floor(
								donationPrices[donationPriceIndex] * ACTUAL_RECEIVED_PERCENTAGE,
							) /
								MONTHLY_COST) *
								100,
						) / 100}
				...
				<button
					...
					onClick={() => {
						isAuthenticated // eslint-disable-line @typescript-eslint/no-unused-expressions
							? useCase
									.makeOneTimeDonation(donationPrices[donationPriceIndex])
			...
			<div className="card">
				...
				<div className={`field ${styles.radioButtonField}`}>
					{radioButtons()}
				</div>
				<button
					className={`button is-fullwidth is-primary ${styles.buttons}`}
					onClick={() => {
						isAuthenticated // eslint-disable-line @typescript-eslint/no-unused-expressions
							? useCase
									.registerSubscription(subscriptionPlanId)
									.then(redirectToCheckout)
