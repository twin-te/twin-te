...
import type { PaymentStatus, PaymentType } from "../../domain/payment_history";
import { fromPBRFC3339DateTime, fromPBUUID } from "./shared";
import { assurePresence } from "./utils";

export const fromPBPaymentUser = (
	pbPaymentUser: DonationV1PB.PaymentUser,
): User => {
	return {
...
export const fromPBSubscription = (
	pbSubscription: DonationV1PB.Subscription,
): Subscription => {
	return {
...
export const fromPBPlan = (
	pbPlan: DonationV1PB.SubscriptionPlan,
): SubscriptionPlan => {
	return {
		id: pbPlan.id,
		name: pbPlan.name,
...
export const fromPBPaymentType = (
	pbPaymentType: DonationV1PB.PaymentType,
): PaymentType => {
...
export const fromPBPaymentStatus = (
	pbPaymentStatus: DonationV1PB.PaymentStatus,
): PaymentStatus => {
...
export const fromPBPaymentHistory = (
	pbPaymentHistory: DonationV1PB.PaymentHistory,
): PaymentHistory => {
	return {
		...
		createdAt: fromPBRFC3339DateTime(
			assurePresence(pbPaymentHistory.createdAt),
		),
	};
};
